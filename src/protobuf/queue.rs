...
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct ConsumeChunkRequest {}
#[derive(Clone, PartialEq, ::prost::Message)]
...
pub mod publishing_queue_client {
    ...
    impl PublishingQueueClient<tonic::transport::Channel> {
    ...
    impl<T> PublishingQueueClient<T>
    where
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> PublishingQueueClient<InterceptedService<T, F>>
        where
        ...
        {
            PublishingQueueClient::new(InterceptedService::new(inner, interceptor))
        }
        ...
        > {
            ...
            let path = http::uri::PathAndQuery::from_static(
                "/queue.PublishingQueue/PublishChunk",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(GrpcMethod::new("queue.PublishingQueue", "PublishChunk"));
...
pub mod publishing_queue_server {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
    ...
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with PublishingQueueServer.
    #[async_trait]
    pub trait PublishingQueue: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct PublishingQueueServer<T> {
        inner: Arc<T>,
    ...
    impl<T> PublishingQueueServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for PublishingQueueServer<T>
    where
        T: PublishingQueue,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/queue.PublishingQueue/PublishChunk" => {
                    ...
                    struct PublishChunkSvc<T: PublishingQueue>(pub Arc<T>);
                    impl<
                        T: PublishingQueue,
                    ...
                    for PublishChunkSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as PublishingQueue>::publish_chunk(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/queue.PublishingQueue/ConsumeChunk" => {
                    ...
                    for ConsumeChunkSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as PublishingQueue>::consume_chunk(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for PublishingQueueServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
